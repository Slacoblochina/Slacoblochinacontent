...
local function status(text, needWait)
	centrizedText(title(), 0xFFFFFF, text)
...
function require(module)
	...
	elseif package.loading[module] then
		...
		local handle, reason = temporaryFilesystemProxy.open(installerPath .. "Windows/System32/" .. module .. ".lua", "rb")
		if handle then
			local data, chunk = ""
			repeat
...
workspace:addChild(GUI.panel(1, 1, workspace.width, workspace.height, 0x2C0091))
