...
local repositoryURL = "https://raw.githubusercontent.com/Slacoblochina/Slacoblochinacontent/main/"
...
local localizationComboBox = GUI.comboBox(1, 1, 22, 1, 0xF0F0F0, 0xFFFFFF, 0xD2D2D2, 0xB4B4B4)
...
addStage(function()
	...
	local function updateDisks()
		...
		local function addDisk(proxy, picture, disabled)
			...
			if disabled then
				picture = image.blend(picture, 0x0xFFFFFF, 0.4)
				disk.disabled = true
			end

			disk:addChild(GUI.image(4, 2, picture))
			disk:addChild(GUI.label(2, 7, disk.width - 2, 1, disabled and 0xFFFFFF or 0x696969, text.limit(proxy.getLabel() or proxy.address, disk.width - 2))):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP)
